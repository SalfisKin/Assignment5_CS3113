...
void initialise()
{
    ...
    g_display_window = SDL_CreateWindow("Hello, Platformers!",
...
void process_input()
{
    ...
    if (g_current_scene->is_a_main_scene == 1) {
            ...
            case SDL_KEYDOWN:
                switch (event.key.keysym.sym) {
                    ...
                    case SDLK_SPACE:
                        ...
                        break;
                    case SDLK_RETURN:
                        if (g_current_scene->is_a_main_scene == 0) {
...
void update()
{
    ...
    if (g_current_scene->m_state.player->get_active_state() == false) {
        life_left--;
        switch_to_scene(g_current_scene);//if there's a collision with the enemy, minus 1 life and restart current scene
    }
